...
> 당시 붙어 있던 Lambda Web Adapter 레이어였다. 6장의 사후 분석을 반드시 읽을 것.
...
8장의 권장 방식(엔드포인트 고정, `--log-type Tail`)으로 측정했다. **두 버전은 메모리가
...
> 3장 기준선(총합 3,523ms)과 직접 비교하지 말 것 — 기준선은 8장에서 설명한 혼합 집단의
> p50이라 측정 대상이 다르다. 위 표는 동일 조건 단건 비교이며 이쪽이 해석 가능한 값이다.

### 메모리 2048 효과 — 미확정

같은 방식으로 2048MB 버전(v47)을 재보니 총합 3,768ms(restore 889ms + 첫 요청 2,879ms)로
1024MB보다 **나쁘게** 나왔다. 그러나 v47은 **갓 발행한 스냅샷**이라 첫 복원이 느린 조건이
섞였다(8장 조건 2 위반). 메모리 효과는 스냅샷이 안정화된 뒤 다시 재야 한다.
...
워밍 핑(4.3)이 낮추는 지표. 기준선 22%. 8장의 함정 때문에 로그 일괄 집계로는 판단할 수
...
2. **갓 발행한 스냅샷과 오래된 스냅샷을 비교하지 말 것.** 새 스냅샷은 첫 복원이 눈에 띄게
   느리다(실측: 새 스냅샷 restore 889ms vs 안정화된 것 621~683ms). 새 버전을 만들었다면
   최소 20분 유휴 후 다시 측정한다.
