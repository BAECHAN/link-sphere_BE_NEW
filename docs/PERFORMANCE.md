...
### 4.1 SnapStart 스냅샷에 워밍업 굽기 — **시도했으나 철회함 (장애 유발)**

> ⚠️ 2026-07-25에 시도했다가 **프로덕션 502 장애를 일으켜 롤백했다.** 아래 6장의
> 사후 분석을 반드시 먼저 읽을 것. 같은 접근을 다시 시도하려면 6장의 제약을 해결해야 한다.

의도: `companion object init`은 체크포인트 **이전**에 실행되므로, 여기서 실제 요청을
흘려보내면 초기화 비용이 스냅샷에 포함된다.

결과: MockMvc로 워밍업하면 **복원 후 두 번째 요청부터 Lambda Web Adapter가
`client error (Connect)`로 죽는다.** 3장에서 진단한 병목은 여전히 유효하지만
이 방법으로는 해결할 수 없다.

### 4.2 메모리 1024 → 2048MB — **현재 1024로 되돌아가 있음**

Lambda는 메모리에 비례해 vCPU를 준다(1024MB ≈ 0.58 vCPU → 2048MB ≈ 1.15 vCPU).
첫 요청은 CPU 바운드라 거의 선형으로 줄어든다. 메모리 자체가 목적이 아니다(사용량 460MB).

이 변경 자체는 무해했다(6장 실험에서 입증). 다만 4.1 롤백 과정에서 1024로 되돌렸고,
아직 다시 올리지 않았다. 단독으로 적용하면 안전하다.
...
## 6. 사후 분석 — 워밍업 시도가 일으킨 502 장애 (2026-07-25)
...
콜드스타트 개선의 핵심이던 워밍업(4.1)이 철회됐으므로 **콜드스타트 수치는 기준선 그대로다.**
현재 적용되어 실제로 효과가 있는 것은 워밍 핑(4.3)뿐이며, 이는 콜드 총합을 줄이는 게 아니라
**콜드가 발생하는 빈도(기준선 22%)** 를 낮춘다.

| 구간 | 현재 | 기준선 | 비고 |
| ---- | ---- | ------ | ---- |
| 콜드 총합 p50 | 3,523ms (변화 없음) | 3,523ms | 워밍업 철회로 미개선 |
| 웜 요청 p50 | 148ms | 148ms | — |
| 콜드 비율 | _(워밍 핑 적용 후 24시간 뒤 재측정)_ | 22% | 유일하게 개선 기대되는 지표 |

**목표(콜드 총합 p50 < 1,500ms)는 미달성 상태다.** 달성하려면 6장의 "다시 시도하려면"을
먼저 해결해야 한다.
